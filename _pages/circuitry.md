...
The board suppports two different I2C addresses. Since we use 2 OLEDs for the robot we need to chnage one of the addresses. If you look at the silkscreen on the back of the board (see image 1) 
![OLED Back](/assets/images/I2C-OLED-back.jpg)
you will see a place where you can select the I2C address by moving a 407ohm surface mount resistor between different contacts. The addresses that you can select are shown on the silkscreen as 0x7A (01111010b) and 0x78 (01111000b). When you scan the I2C bus you will not see any devices withthese addresses but rather 0x3D (00111101b) and 0x3C (00111100b). If you look carefully you will see that the second 2 addresses are just the firts 2 addresses bit shifted to the right one bit. The reason for this is that in common I2C communication the first seven bits of the "address byte" contains the device address and the last bit is a read/write bit. 0 is write and 1 is read.
